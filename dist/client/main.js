...
let state = { players: {}, bullets: [], mines: [], map: [] };
...
network.onState = (msg) => {
    ...
    if (me) {
        ...
        (0, ui_1.updateUI)(me.lives, me.cooldown, playerCount, mineCount);
...
function gameLoop() {
    ...
    for (const id in state.players) {
        const p = state.players[id];
        if (p.lives <= 0 || Date.now() < p.deadUntil)
            continue;
        ctx.save();
    ...
    explosionEffects = explosionEffects.filter(effect => {
        ...
        if (age > 200)
            return false; // Remove after 200ms
        ctx.save();
