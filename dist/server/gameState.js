...
class GameState {
    constructor(config) {
        ...
        this.config = config;
        this.map = (0, mapGenerator_1.generateMap)(config);
    }
    addPlayer() {
    ...
    handleMessage(playerId, msg) {
        ...
        if (!player || player.lives <= 0 || Date.now() < player.deadUntil)
            return;
        switch (msg.type) {
            case "move":
    ...
    update() {
        ...
        for (const id in this.players) {
            ...
            player.mines = player.mines.filter((mine) => this.mines.some((m) => m.id === mine.id));
        }
    }
    startGameLoop() {
    ...
    _checkObstacleCollision(x, y, radius) {
